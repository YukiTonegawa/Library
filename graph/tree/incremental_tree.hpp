...
struct incremental_tree {
    ...
    int make_node(int p) {
        ...
        par.push_back({});
        par[id][0] = p;
        for (int i = 0; i + 1 < max_nodecount_log_half; i++) {
            par[id][i + 1] = par[par[par[par[id][i]][i]][i]][i];
        }
        return id;
    }
    ...
    // depth(v) < kの場合0
    int la(int v, int k) {
        for (int i = max_nodecount_log_half - 1; i >= 0; i--) {
            while (k >= (1 << (i << 1))) v = par[v][i], k -= 1 << (i << 1);
        }
        return v;
    }
    ...
    int lca(int u, int v) {
        ...
        u = la(u, depdiff);
        if (u == v) return u;
        for (int i = max_nodecount_log_half - 1; i >= 0 && u != v; i--) {
            while (par[u][i] != par[v][i]) u = par[u][i], v = par[v][i];
        }
        return par[u][0];
    }
