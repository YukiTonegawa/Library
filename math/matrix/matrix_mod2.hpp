...
#include <vector>
#include <iostream>
#include "../../data_structure/bit/_dynamic_bitset.hpp"
...
struct matrix_mod2 {
  ...
  private:
    ...
    // K == 0だと壊れる
    // O(NKM / w)
    static matrix mul_mat(const matrix &vl, const matrix &vr) {
    ...
    static void add_mat_inplace(matrix &vl, const matrix &vr) {
  ...
  public:
    ...
    matrix_mod2 operator *  (const matrix_mod2 &vr) { return mul_mat(*this, vr); }
    matrix_mod2 operator ^  (const long long vr) { return pow(vr); }
    matrix_mod2 operator += (const matrix_mod2 &vr) { add_mat_inplace(*this, vr); return *this; }
    matrix_mod2 operator *= (const matrix_mod2 &vr) { return (*this) = mul_mat(*this, vr); }
    ...
    // O(n^3 log k / w)
    matrix_mod2 pow(long long k) const {
    ...
    bool det() {
        ...
        for (int i = 0; i < n; i++) {
            if (((tmp.val[i][i >> bitlen_shift] >> (i & bitlen_mod)) & 1) == 0) {
                return false;
            }
        }
        return true;
    ...
    // (n, m) + (l, m) -> (n + l, m) 縦に結合
    matrix_mod2 concat_vertical(matrix_mod2 vr) const {
        assert(m == vr.m);
    ...
    // (n, m) + (n, l) -> (n, m + l)　横に結合
    matrix_mod2 concat_horizontal(matrix_mod2 vr) const {
        assert(n == vr.n);
        matrix_mod2 ret(*this);
    ...
    // (n, m) -> (k, m), (n - k, m)
    std::pair<matrix_mod2, matrix_mod2> split_vertical(int k) const {
        assert(0 <= k && k <= n);
    ...
    // (n, m) -> (n, k), (n, m - k)
    std::pair<matrix_mod2, matrix_mod2> split_horizontal(int k) const {
        assert(0 <= k && k <= m);
    ...
    // 解だけ知りたい場合
    static std::pair<int, dynamic_bitset> system_of_linear_equations_sol(const matrix_mod2 &A, const dynamic_bitset &b) {
        assert(b.n == A.n);
        matrix_mod2 tmp = A.concat_horizontal(matrix_mod2(A.n, b).t()).gaussian_elimination().first;
        //解空間の次元 = 変数の数 - 階数
        int r = tmp.rank();
    ...
    static std::tuple<int, matrix_mod2, dynamic_bitset> system_of_linear_equations(const matrix_mod2 &A, const dynamic_bitset &b) {
        assert(b.size() == A.n);
        matrix_mod2 tmp = A.concat_horizontal(matrix_mod2(A.n, b).t()).gaussian_elimination().first;
        //解空間の次元 = 変数の数 - 階数
        int r = tmp.rank();
        ...
        for (int i = 0; i < r; i++) {
            ...
            for (int j = i; j < tmp.m; j++) {
                if (tmp.get(i, j) == 0) continue;
                if (j == tmp.m - 1 && !f) {
                    return {-1, matrix_mod2(0, 0), dynamic_bitset(0)}; // 解なし
                }
                if (!f) { 
                    fc[i] = j;
                    f = true;
        ...
        matrix_mod2 res(d, v, 0);
        std::vector<bool> not_fc(v, true);
        for (int i = 0; i < r; i++) not_fc[fc[i]] = false;
        for (int i = 0, j = 0; i < v; i++) if (not_fc[i]) res.set(j++, i, 1);


        for (int i = r - 1; i >= 0; i--){ //各行に1つまだ確定していない変数が現れる
            int col = fc[i];
            assert(col != -1);
            assert(tmp.get(i, col));
            for (int k = 0; k < d; k++) { // 次元
                int L = (col + 1) >> bitlen_shift, R = (v + bitlen - 1) >> bitlen_shift;
                int Lrem = (col + 1) & bitlen_mod, Rrem = (R << bitlen_shift) - v;
                ull bit = 0;
                for (int j = L; j < R; j++) {
                    ull x = res.val[k][j] & tmp.val[i][j];
                    if (j == L) {
                        x >>= Lrem;
                        x <<= Lrem;
                    }
                ...
                int f = res.get(k, col) + __builtin_popcountll(bit);
                res.set(k, col, f & 1);
            }
        }
        return {d, res, plus};
