...
struct matrix_mod {
  ...
  private:
    ...
    static void sub_mat_inplace(matrix &vl, const matrix &vr) {
        assert(vl.n == vr.n && vl.m == vr.m);
        int N = vl.n, M = vl.m;
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
    ...
    static void mul_val_inplace(matrix &vl, mint vr) {
        int N = vl.n, M = vl.m;
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                vl[i][j] *= vr;
    ...
    static void add_val_inplace(matrix &vl, mint vr) {
        int N = vl.n, M = vl.m;
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                vl[i][j] += vr;
    ...
    static void sub_val_inplace(matrix &vl, mint vr) {
        int N = vl.n, M = vl.m;
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                vl[i][j] -= vr;
  ...
  public:
    ...
    matrix_mod operator -  (const matrix_mod &vr) { matrix_mod tmp(*this); return tmp -= vr; }
    matrix_mod operator *  (const matrix_mod &vr) { return mul_mat(*this, vr); }
    matrix_mod operator ^  (const long long vr) { return pow(vr); }
    matrix_mod operator *  (const mint vr) { matrix_mod tmp(*this); return tmp *= vr; }
    matrix_mod operator += (const matrix_mod &vr) { add_mat_inplace(*this, vr); return *this; }
    matrix_mod operator -= (const matrix_mod &vr) { sub_mat_inplace(*this, vr); return *this; }
    matrix_mod operator *= (const matrix_mod &vr) { return (*this) = mul_mat(*this, vr); }
    ...
    matrix_mod pow(long long k) const {
    ...
    // (n, m) + (n, l) -> (n, m + l)　横に結合
    matrix_mod concat_horizontal(matrix_mod vr) const {
        assert(n == vr.n);
        matrix_mod res(*this);
        for (int i = 0; i < n; i++) {
    ...
    // (n, m) + (l, m) -> (n + l, m) 縦に結合
    matrix_mod concat_vertical(matrix_mod vr) const {
        assert(m == vr.m);
        matrix_mod res(*this);
    ...
    // (n, m) -> (n, k), (n, m - k)
    std::pair<matrix_mod, matrix_mod> split_horizontal(int k) const {
        assert(0 <= k && k <= m);
    ...
    // (n, m) -> (k, m), (n - k, m)
    std::pair<matrix_mod, matrix_mod> split_vertical(int k) const {
        assert(0 <= k && k <= n);
    ...
    std::pair<matrix_mod, matrix_mod> lu_decomposition() const {
    ...
    static std::tuple<int, matrix_mod, vec> system_of_linear_equations(const matrix_mod &A, const vec &b) {
        assert(b.size() == A.n);
        matrix_mod tmp = A.concat_horizontal(matrix_mod(b).t()).gaussian_elimination().first;
        //解空間の次元 = 変数の数 - 階数
        int r = tmp.rank();
