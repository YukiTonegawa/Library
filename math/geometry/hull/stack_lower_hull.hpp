...
struct stack_lower_hull {
  ...
  private:
    ...
    std::vector<point> P;
    std::vector<int> sz;
    bool ccw(int a, int b, int c) { return point::cross(P[a], P[b], P[c]) < 0; }
    // n <= 2^kとなるような最小のk
    int ceillog2(int n) {
        if (n == 0) return 0;
  ...
  public:
    ...
    // O(logN)
    std::pair<int, T> linear_add_min(int r, T a, T b) {
        assert(0 < r && r <= size_points());
        int pos = r - 1;
    ...
    // [0, i]の点だけで作った下側凸包のうちp以上の最小, ない場合は-1
    ...
    // [0, i]の点だけで作った下側凸包のうちp以下の最大, ない場合は-1
