#ifndef _LI_CHAO_TREE_ROLLBACK_H_
#define _LI_CHAO_TREE_ROLLBACK_H_
#include <algorithm>
#include <vector>
#include "../base/bit_ceil.hpp"

template<typename T>
struct li_chao_tree_rollback {
  ...
  public:
    ...
    // xはソート済みかつユニーク
    li_chao_tree_rollback(const std::vector<T> &x) : N(x.size()), M(bit_ceil(N)), L(2 * M, line(0, inf)), sample_points(x) {
    ...
    void add_line(T a, T b) {
        history_begin.push_back(history.size());
        line x(a, b);
    ...
    void rollback() {
        assert(!history_begin.empty());
        int len = history_begin.back();
        history_begin.pop_back();
        while (history.size() > len) {
            auto [k, l] = history.back();
            history.pop_back();
            L[k] = l;
        }
    }
