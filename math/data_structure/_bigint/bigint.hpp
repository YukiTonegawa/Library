...
struct bigint : std::vector<unsigned long long>{
  bool s;
  int exp_10 = 0;
private:
  ...
  static std::vector<unsigned long long> convolution_bigint(const std::vector<unsigned long long> &a, const std::vector<unsigned long long> &b){
  ...
  static std::vector<unsigned long long> square_bigint(const std::vector<unsigned long long> &a){
  ...
  static void sub(bigint &a, bigint b){
    int e = std::min(a.exp_10, b.exp_10);
    if(e < a.exp_10){
      a.lshift_inplace(a.exp_10 - e);
      a.exp_10 = e;
  ...
  bigint inv_newton(int p)const{
    ...
    x.exp_10 = -(d + 1);
    bigint pre = __prefix(base_log_10);
    for(int i = 0; i < 5; i++){
    ...
    x = x.__prefix(base_log_10);
    for(int i = 1; i < p; i <<= 1){
      bigint y = __prefix(std::max(base_log_10, i << 1)) * x.square();
      x = (x + x - y);
  ...
  // 先頭d桁
  bigint __prefix(int d)const{
    int deg = __deg();
    int n = std::min(deg, d);
    bigint res = rshift(deg - n);
    res.exp_10 += deg - n;
    return res;
  }
...
public:
  ...
  std::pair<bigint, bigint> div2(const bigint &vr)const{
    bigint q = bigint(*this).div1(vr);
    q = q.rshift(-q.exp_10);
    q.exp_10 = 0;
    bigint r = *this - (vr * q);
    if(vr <= r){
  ...
  bigint operator *= (const bigint &r){
    ...
    s ^= r.s;
    exp_10 += r.exp_10;
    auto v = convolution_bigint(*this, r);
    int n = v.size();
    this->resize(n);
    for(int i = 0; i < n; i++) (*this)[i] = v[i];
    return *this;
  }
  bigint operator /= (const bigint &vr){return *this = div2(vr).first;}
  bigint operator %= (const bigint &vr){return *this = div2(vr).second;}
  bigint operator + (const bigint& vr)const{return bigint(*this) += vr;}
  bigint operator - (const bigint& vr)const{return bigint(*this) -= vr;}
  bigint operator * (const bigint& vr)const{return bigint(*this) *= vr;}
  bigint operator / (const bigint &vr)const{return div2(vr).first;}
  bigint operator % (const bigint &vr)const{return div2(vr).second;}
  bigint operator += (long long r){return *this += bigint(r);}
  bigint operator -= (long long r){return *this -= bigint(r);}
  bigint operator *= (long long r){return *this *= bigint(r);}
  bigint operator /= (long long r){return *this /= bigint(r);}
  bigint operator %= (long long r){return *this %= bigint(r);}
  bigint operator + (long long r){return bigint(*this) += bigint(r);}
  bigint operator - (long long r){return bigint(*this) -= bigint(r);}
  bigint operator * (long long r){return bigint(*this) *= bigint(r);}
  bigint operator / (long long r){return bigint(*this) /= bigint(r);}
  bigint operator % (long long r){return bigint(*this) %= bigint(r);}
  bool operator == (const bigint &vr)const{
