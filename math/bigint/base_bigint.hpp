#ifndef _BASE_BIGINT_H_
#define _BASE_BIGINT_H_
...
struct base_bigint {
    ...
    static std::string to_string(const std::vector<i64> &a) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            if (a[i] && !nonzero) {
                ...
                res += std::string(base_log - t.size(), '0') + t;
            }
        }
        if (!nonzero) res = '0';
        return res;
        ...
        for (int i = n - 1; i >= 0; i--) {
            if (a[i]) {
                for (int j = base_log - 1; j >= 0; j--) {
                    if (a[i] >= pow10[j]) return i * base_log + j;
                }
            }
        }
        return -1; // 0
    ...
    static std::vector<i64> lshift(const std::vector<i64> &a, int k) {
        ...
        if (r == 0) {
            ...
            for (int i = n - 1; i > s; i--) {
                res[i] = (i - s >= a.size() ? 0 : a[i - s]) % pow10[m] * pow10[r] + a[i - s - 1] / pow10[m];
            }
            res[s] = (a[0] % pow10[m]) * pow10[r];
        }
        return res;
    }
    ...
    static std::vector<i64> rshift(const std::vector<i64> &a, int k) {
        ...
        if (r == 0) {
            ...
            int n = int(res.size()), m = base_log - r;
            for (int i = 0; i < n - 1; i++) {
                res[i] = a[i + s] / pow10[r] + a[i + s + 1] % pow10[r] * pow10[m];
            }
            res[n - 1] = a[n - 1 + s] / pow10[r];
        }
        return res;
    }
    ...
    struct mul_bigint {
        ...
        static std::vector<i64> mul_naive(const std::vector<i64> &a, const std::vector<i64> &b) {
            ...
            return base_bigint::modify<i128>(c);
        }
