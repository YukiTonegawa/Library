...
struct ds_avl_persistent {
  private:
    ...
    ds_avl_persistent(node *v) : root(v) {}
  public:
    ds_avl_persistent() : root(nullptr) {}
    ds_avl_persistent(const std::string &S) : ds_avl_persistent(stovector(S)){}
    template<typename T>
    ds_avl_persistent(const std::vector<T> &v) {
        if (v.empty()) {
            root = nullptr;
            return;
        }
    ...
    template<typename T>
    self_t set(int k, T x) const {
        assert(0 <= k && k < size());
        return self_t(set_inner(root, k, rh::to_hash(x)));
    }
    ...
    self_t insert(int k, ull x) {
        assert(0 <= k && k <= size());
        return self_t(insert_inner(root, k, x));
    }

    self_t erase(int k) {
        assert(0 <= k && k < size());
        return self_t(erase_inner(root, k));
    }
...
typename ds_avl_persistent<MAX_LEN>::node *ds_avl_persistent<MAX_LEN>::tmp_node = nullptr;
#endif