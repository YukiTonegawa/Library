#ifndef _WAVELET_TREE_COMPRESSED_H_
#define _WAVELET_TREE_COMPRESSED_H_
#include <vector>
#include "wavelet_tree.hpp"
#include "../../base/compressor.hpp"

template<typename T>
struct wavelet_tree_compressed {
  private:
    std::vector<T> rev;
    compressor<T> cmp;
  ...
  public:
    wavelet_tree_compressed() {}
    wavelet_tree_compressed(const std::vector<T> &_v) {
    ...
    template<typename F>
    wavelet_tree_compressed(const std::vector<T> &_v, F f) {
        cmp.add(_v.begin(), _v.end());
        int N = _v.size();
        std::vector<int> v(N);
        for (int i = 0; i < N; i++) v[i] = cmp.ord(_v[i]);
