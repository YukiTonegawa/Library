#ifndef _MULTIMAP_MONOID_LAZY_H_
#define _MULTIMAP_MONOID_LAZY_H_
#include <vector>
#include <cassert>
#include <iostream>

// キーの昇順, キーが同じものは追加した順番の昇順
template<typename Key, typename S, S (*op)(S, S), S (*e)(), typename F, S (*mapping)(F, S, int), F (*composition)(F, F), F (*id)()>
struct multimap_monoid_lazy{
private:
  struct node{
    ...
    node(Key _key, S _val): h(1), sz_unique(1), sz_sum(1), key(_key), val(_val), sum(_val), lazy(id()), l(nullptr), r(nullptr), p(nullptr), lmost(this), rmost(this){}
    int balanace_factor(){
  ...
  node *emplace_inner(node *v, Key k, S val){
  ...
  // kの要素がすでにある場合何もしない
  node *emplace_unique_inner(node *v, Key k, S val){
    if(!v) return new node(k, val);
    push_down(v);
  ...
  node *emplace_replace_inner(node *v, Key k, S val){
    if(!v) return new node(k, val);
    push_down(v);
  ...
  void update_inner(node *v, Key l, Key r, F x){
  ...
  S query_inner(node *v, Key l, Key r){
    if(!v || v->rmost->key < l || r <= v->lmost->key) return e();
  ...
  template<typename G>
  node *max_right(node *v, G &f, Key l, S &ok){
    ...
    node *x = max_right(v->l, f, l, ok);
    if(x) return x;
    if(l <= v->key){
      ok = merge(ok, v->val);
      if(f(ok)) return v;
    }
    return max_right(v->r, f, l, ok);
  }
  template<typename G>
  node *min_left(node *v, G &f, Key r, S &ok){
    ...
    node *x = min_left(v->r, f, r, ok);
    if(x) return x;
    if(v->key <= r){
      ok = merge(v->val, ok);
      if(f(ok)) return v;
    }
    return min_left(v->l, f, r, ok);
  }
  void to_list_inner(node *v, std::vector<std::pair<Key, S>> &res){
    push_down(v);
...
public:
  multimap_monoid_lazy(): root(nullptr){}
  multimap_monoid_lazy(std::vector<std::pair<Key, S>> v){
    std::sort(v.begin(), v.end());
    init_sorted(v);
  }
  // 1
  // すでにソート済み
  void init_sorted(const std::vector<std::pair<Key, S>> &v){
    if(v.empty()){
      root = nullptr;
      return;
    }
  ...
  void emplace(Key k, S val){
    root = emplace_inner(root, k, val);
    root->p = nullptr;
  }
  void emplace_unique(Key k, S val){
    root = emplace_unique_inner(root, k, val);
    root->p = nullptr;
  }
  void emplace_replace(Key k, S val){
  ...
  template<typename G>
  std::pair<node*, S> max_right(Key l, G f){
    S ok = e();
    node* ret = max_right(root, f, l, ok);
    return {ret, ok};
  }
  template<typename G>
  std::pair<node*, S> min_left(Key r, G f){
    S ok = e();
    node* ret = min_left(root, f, r, ok);
    return {ret, ok};
  }
  std::vector<std::pair<Key, S>> to_list(){
    std::vector<std::pair<Key, S>> res;
    if(root) to_list_inner(root, res);
    return res;
  }
  static void swap(multimap_monoid_lazy &A, multimap_monoid_lazy &B){
