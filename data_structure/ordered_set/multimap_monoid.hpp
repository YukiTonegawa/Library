...
struct multimap_monoid {
    struct node {
        ...
        node(Key _key, S _val): h(1), sz_unique(1), sz_sum(1), key(_key), val(_val), sum(_val), l(nullptr), r(nullptr), p(nullptr), lmost(this), rmost(this) {}
        int balanace_factor() {
  ...
  S prod_inner(node *v, Key l, Key r){
    if(!v || v->rmost->key < l || r <= v->lmost->key) return e();
    if(l <= v->lmost->key && v->rmost->key < r) return v->sum;
    if(r <= v->key) return prod_inner(v->l, l, r);
    if(v->key < l) return prod_inner(v->r, l, r);
    return op(prod_inner(v->l, l, r), op(v->val, prod_inner(v->r, l, r)));
  }
  template<typename F>
  node *max_right(node *v, F &f, Key l, S &ok){
  ...
  template<typename F>
  node *min_left(node *v, F &f, Key r, S &ok){
...
public:
  ...
  multimap_monoid(std::vector<std::pair<Key, S>> v){
  ...
  // 1
  template<typename F>
  std::pair<node*, S> max_right(Key l, F f){
    S ok = e();
    node* ret = max_right(root, f, l, ok);
    return {ret, ok};
  }
  template<typename F>
  std::pair<node*, S> min_left(Key r, F f){
    S ok = e();
  ...
  static void swap(multimap_monoid &A, multimap_monoid &B){
