#ifndef _PERSISTENT_MULTISET_H_
#define _PERSISTENT_MULTISET_H_
#include <vector>
#include <cassert>
#include <numeric>
...
struct persistent_multiset {
  private:
    using self_t = persistent_multiset<T>;
    ...
    int rootid;
    persistent_multiset(int _rootid) : rootid(_rootid) {}
    
  public:
    // Aがソート済
    persistent_multiset(const std::vector<T> &A) : rootid(build(A, 0, A.size())) {}
    persistent_multiset() : rootid(nullid) {}
...
template<typename T>
std::vector<typename persistent_multiset<T>::node> persistent_multiset<T>::V;
template<typename T>
int persistent_multiset<T>::erased_node;
#endif