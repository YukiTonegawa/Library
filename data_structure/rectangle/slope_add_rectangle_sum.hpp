#ifndef _SLOPE_ADD_RECT_SUM_H_
#define _SLOPE_ADD_RECT_SUM_H_
#include <array>
#include <vector>

template<typename Idx, typename Val>
struct slope_add_rectangle_sum_static {
  private:
    struct bit6 {
        int N;
        ...
        void apply(Idx l, Idx r, int lc, int rc, Val z1, Val z2, Val z3) {
        ...
        std::tuple<Val, Val, Val> prod(Idx r, int rc) {
        ...
        std::tuple<Val, Val, Val> prod(Idx l, Idx r, int lc, int rc) {
            auto [cr, dxr, dx2r] = prod(r, rc);
            auto [cl, dxl, dx2l] = prod(l, lc);
    ...
    void solve(std::vector<Val> &ans) {
        ...
        while (qid < 2 * M) {
            if (uid < 2 * N && U[uid].lx < Q[qid].lx) {
                ...
                Val c = U[uid].z2 * i2;
                bit.apply(U[uid].ly, U[uid].ry, U[uid].lyc, U[uid].ryc, a, b, c);
                uid++;
            } else {
                auto [a, b, c] = bit.prod(Q[qid].ly, Q[qid].ry, Q[qid].lyc, Q[qid].ryc);
                int id = Q[qid].id;
                if (id >= M) {
  ...
  public:
    slope_add_rectangle_sum_static() {}

    // [lx, rx) × [ly, ry)にax + bを足す
    void apply(Idx lx, Idx rx, Idx ly, Idx ry, Val a, Val b) {
