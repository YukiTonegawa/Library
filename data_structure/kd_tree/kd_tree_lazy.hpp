#ifndef _KD_TREE_LAZY_H_
#define _KD_TREE_LAZY_H_
#include <numeric>
#include <vector>
#include <algorithm>
#include <cassert>

template<typename Idx, typename S, S (*op)(S, S), S (*e)(), typename F, S (*mapping)(F, S), F (*composition)(F, F), F (*id)()>
struct kd_tree_lazy {
  public:
    struct point {
        Idx x, y;
  ...
  private:
    ...
    struct node {
        ...
        F lz;
        S sum;
        node () : lx(inf), rx(minf), ly(inf), ry(minf), r(0), lz(id()), sum(e()) {}
        node (point p) : lx(p.x), rx(p.x), ly(p.y), ry(p.y), r(0), lz(id()), sum(p.z) {}
    };
    ...
    void push(int v) {
        if (V[v].lz != id()) {
            all_apply(v + 1, V[v].lz);
            all_apply(V[v].r, V[v].lz);
            V[v].lz = id();
        }
    }
  ...
  public:
    kd_tree_lazy() {}
    kd_tree_lazy(const std::vector<point> &v) : Leaf(v.size()) { 
        if(!v.empty()) {
    ...
    template<typename Idx2>
    void apply_euclid(Idx x, Idx y, Idx d, F lz) {
        if (root >= V.size()) return e();
        int pos = 0;
        st[0] = root;
        while(pos >= 0) {
    ...
    void apply_manhattan(Idx x, Idx y, Idx d, F lz) {
        if (root >= V.size()) return e();
        int pos = 0;
        st[0] = root;
        while(pos >= 0) {
