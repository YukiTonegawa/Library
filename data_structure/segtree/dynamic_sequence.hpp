...
template<typename S, S (*op)(S, S), S (*e)(), S (*flip_val)(S)>
struct dynamic_sequence {
  private:
    using self_t = dynamic_sequence<S, op, e, flip_val>;
    struct V {
        S val, sum;
        int size;
        bool _flip;
        V(S s) : val(s), sum(s), size(1), _flip(false) {}
    };
    ...
    node *root;
    dynamic_sequence(node *v) : root(v) {}
  public:
    ...
    dynamic_sequence(const std::vector<S> &_v) : root(nullptr) {
        int N = _v.size();
